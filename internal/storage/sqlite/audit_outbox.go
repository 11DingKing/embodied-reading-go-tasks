...
func (s *Store) ClaimOutbox(ctx context.Context, owner string, now time.Time, ttl time.Duration) (repository.OutboxEvent, error) {
	s.outboxLock.Lock()
	defer s.outboxLock.Unlock()

	row := s.q.QueryRowContext(ctx, `SELECT id, tenant_id, topic, aggregate_id, payload, state, attempt, next_try_at, lease_owner, lease_token, lease_until, last_error, created_at, updated_at
        FROM outbox_claim_candidates WHERE state IN (?, ?) AND next_try_at <= ? AND (lease_until IS NULL OR lease_until <= ?)
	...
	_, err = s.q.ExecContext(ctx, `UPDATE outbox_events SET state=?, lease_owner=?, lease_token=lease_token+1, lease_until=?, attempt=attempt+1, updated_at=?
        WHERE id=?`, repository.OutboxLeased, owner, formatTime(now.Add(ttl)), formatTime(now), value.ID)
	if err != nil {
		return repository.OutboxEvent{}, mapSQLError("claim outbox event", err)
	}
