...
func TestAtomicMemberBatchRollsBackEarlierItems(t *testing.T) {
	...
	defer db.Close()
	trigger := fmt.Sprintf(`CREATE TRIGGER reject_second_member BEFORE INSERT ON program_members WHEN NEW.user_id='%s' BEGIN SELECT RAISE(ABORT, 'member unavailable'); END`, second)
